...
let make_controller (v: 'a) : 'a value_controller =
  ...
  let valu = ref v in
  {add_change_listener = (fun f -> listeners.contents 
                          <- f :: listeners.contents);
   get_value = (fun () -> valu.contents);
...
let checkbox (init: bool) (s: string) : widget * bool value_controller =
  ...
  let widg = {repaint = (fun (g: Gctx.gctx) -> 
                if vcontrol1.get_value() then
                  (Gctx.draw_string g (0,0) "X")
                else ());
              handle = (fun (g: Gctx.gctx) (e: Gctx.event) -> ());
              size = fun () -> (fst (Gctx.text_size "X"), 
                                snd (Gctx.text_size "X"))
                } in
  let (lbl', nc) = notifier (hpair (border widg) lbl) in
  nc.add_event_listener (mouseclick_listener (fun f -> 
                                              vcontrol1.change_value (not 
                                              (vcontrol1.get_value()))));
  (lbl', vcontrol1)
...
let radio_button (init: bool) (s: string) : 
                               widget * notifier_controller * 
                               bool value_controller * label_controller =
  let vc = make_controller init in 
  let (lbl, lc) = label s in
  let widg = {repaint = (fun (g: Gctx.gctx) -> 
                Gctx.draw_ellipse g (10, 10) 10 10;
                if vc.get_value() then
                  Gctx.fill_ellipse g (10, 10) 7 7
                else ()); 
              handle = (fun (g: Gctx.gctx) (e: Gctx.event) -> ());
              size = fun () -> (20, 20)
                } in
  let (lbl', nc) = notifier (hpair (border (hpair widg lbl)) (space (5,5))) in
  (lbl', nc, vc, lc)

(** Creates a group of radio buttons and its controller 
    using list of string labels and initial state*)
let radio_button_group (labels: string list) 
                       (init: string) : widget * string value_controller =
  let vcontrol2 = make_controller init in
  let rb_group : (widget * notifier_controller
                  * bool value_controller * label_controller) list =
    List.map (fun s -> radio_button (s = init) s) labels in
  let widlist = List.map (fun (w, nc, vc, lc) -> w) rb_group in
  List.iter (fun (w, nc, vc, lc) -> nc.add_event_listener 
                                     (mouseclick_listener 
                                     (fun f -> vcontrol2.change_value 
                                      (lc.get_label())))) rb_group;
  vcontrol2.add_change_listener (fun f -> (List.iter (fun (w, nc, vc, lc) ->
                                           if f = lc.get_label()
                                           then vc.change_value true 
                                           else vc.change_value false) 
                                           rb_group));
  let widg = hlist widlist in
  (widg, vcontrol2)  
